...
function _sampleLexiconLabel(experience, vocabSize = 5) {
  ...
  let r = Math.random() * total, cumulative = 0;
...
function _nextState(current, pStable, pLexicon, pExplore) {
  const r = Math.random();
  if (current === 'lexicon') {
...
function decide(experience, {
  maxTokens = 8, pStable = 0.8, pLexicon = 0.1, pExplore = 0.05, ergodicState = null
} = {}) {
  ...
  const next = _nextState(state, pStable, pLexicon, pExplore);
  ...
  if (next === 'core') {
    ...
    const label = _sampleLexiconLabel(experience);
    tokens = [experience.selfToken.token, 'notice', label || 'familiar'];
  } else if (next === 'explore') {
    tokens = [CORE_TOKENS[Math.floor(Math.random() * CORE_TOKENS.length)]];
  } else {
    tokens = ['wait'];
  }
...
module.exports = { decide, _sampleLexiconLabel };
